...
const submitForm = () => {
  if (props.note) {
    form.put(route('jobs.notes.update', [props.job.id, props.note.id]), {
      onSuccess: () => {
        success('Note Updated', 'Job note has been updated successfully.');
        emit('success');
      },
      onError: () => {
    ...
    form.post(route('jobs.notes.store', props.job.id), {
      onSuccess: () => {
