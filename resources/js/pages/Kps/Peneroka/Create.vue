            ...
            <section class="flex flex-col gap-5 2xl:flex-row 2xl:items-end 2xl:justify-between">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-4 max-w-2xl text-base leading-7 text-[#65534d] md:text-lg">
                        Register a new beneficiary into {{ site.name }} and keep the site data tidy from day one.
                    </p>
                </div>
            ...
            <div class="grid gap-6 lg:grid-cols-[1.4fr,0.9fr]">
                <Card class="rounded-[34px] border-[#efdcd5] bg-white/92 shadow-[0_18px_50px_rgba(157,80,53,0.08)]">
                    <CardHeader class="border-b border-[#f0dfd8] pb-6">
                        <CardTitle class="text-xl font-black text-[#1b1b1b]" style="font-family: Manrope, Inter, sans-serif;">Peneroka details</CardTitle>
                        <p class="text-sm text-[#7f675f]">Fill in the minimal identity and contact data for this site record.</p>
                    </CardHeader>
                    ...
                    <CardContent class="pt-6">
                        <form class="space-y-5" @submit.prevent="submit">
                            <div class="grid gap-2">
                                ...
                                <Input id="name" v-model="form.name" placeholder="Full name" class="rounded-2xl border-[#e6d1c7] bg-[#fffaf8] px-4 py-3" />
                            ...
                            <div class="grid gap-2 md:grid-cols-2">
                                <div class="grid gap-2">
                                    ...
                                    <Input id="ic_number" v-model="form.ic_number" placeholder="Optional identity number" class="rounded-2xl border-[#e6d1c7] bg-[#fffaf8] px-4 py-3" />
                                    <InputError :message="form.errors.ic_number" />
                                </div>
                                <div class="grid gap-2">
                                    <Label for="phone" class="text-sm font-semibold text-[#2a2422]">Phone</Label>
                                    <Input id="phone" v-model="form.phone" placeholder="Contact number" class="rounded-2xl border-[#e6d1c7] bg-[#fffaf8] px-4 py-3" />
                            ...
                            <div class="grid gap-2">
                                ...
                                <Textarea id="address" v-model="form.address" rows="5" placeholder="Mailing or site address" class="rounded-3xl border-[#e6d1c7] bg-[#fffaf8] px-4 py-3" />
                ...
                <Card class="rounded-[34px] border-[#efdcd5] bg-white/88 shadow-[0_18px_50px_rgba(157,80,53,0.08)]">
                    <CardHeader class="border-b border-[#f0dfd8] pb-6">
                        <CardTitle class="text-xl font-black text-[#1b1b1b]" style="font-family: Manrope, Inter, sans-serif;">Site context</CardTitle>
                    </CardHeader>
                    <CardContent class="space-y-4 pt-6">
                        <div class="rounded-[24px] bg-[#171717] px-4 py-4 text-white shadow-[0_16px_30px_rgba(23,23,23,0.2)]">
                            <p class="text-[11px] font-bold uppercase tracking-[0.24em] text-white/55">Site</p>
                            <p class="mt-1 text-lg font-semibold">{{ site.name }}</p>
                            <p class="text-sm text-white/70">{{ site.code }}</p>
                        </div>

                        <div class="space-y-3 text-sm text-[#65534d]">
                            <p>Use this form for new records only. Existing peneroka should be edited from the registry.</p>
                            <p>Identity and phone details improve monthly recovery and statement visibility later in the workflow.</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
