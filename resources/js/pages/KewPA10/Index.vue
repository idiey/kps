...
import { Plus, Search, Filter, CheckCircle, XCircle, Clock } from 'lucide-vue-next'
...
const hasActiveFilters = computed(() => {
  return !!(search.value || selectedDepartment.value || selectedPriority.value ||
            selectedVerified.value || dateFrom.value || dateTo.value)
})
  ...
  <AppLayout>
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold tracking-tight">KEW.PA-10 Forms</h1>
          <p class="text-muted-foreground">
            Manage government procurement and repair authorization forms
          </p>
        </div>
        <Link href="/kew-pa-10/create">
          <Button>
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <Button variant="outline" @click="showFilters = !showFilters">
                ...
                Filters
                <Badge v-if="hasActiveFilters" variant="default" class="ml-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Showing {{ kewPA10s.data.length }} of {{ kewPA10s.total }} forms
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              <TableRow v-for="form in kewPA10s.data" :key="form.id">
                ...
                <TableCell>
                  <div class="text-sm">
                    <div class="font-medium">{{ form.asset.asset_name }}</div>
                    <div class="text-muted-foreground">{{ form.asset.asset_tag }}</div>
                  </div>
                </TableCell>
                <TableCell>
                  <div class="max-w-xs truncate" :title="form.description">
                    {{ form.description }}
                  </div>
                </TableCell>
                <TableCell>
                  <Badge :variant="getPriorityVariant(form.priority)">
                    {{ form.priority }}
                  </Badge>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Link
                    ...
                    :href="`/jobs/${form.workshop_job.id}`"
          ...
          <!-- Pagination -->
          <div v-if="kewPA10s.last_page > 1" class="flex items-center justify-between mt-6">
            <div class="text-sm text-muted-foreground">
              Showing {{ ((kewPA10s.current_page - 1) * kewPA10s.per_page) + 1 }} to
              {{ Math.min(kewPA10s.current_page * kewPA10s.per_page, kewPA10s.total) }} of
              {{ kewPA10s.total }} results
            </div>
            <div class="flex gap-2">
              <Link
