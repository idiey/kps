...
const totalCost = computed(() => {
  return parts.value.reduce((sum, part) => sum + (part.quantity * part.cost), 0)
})

// Check photo requirements
const hasAllRequiredPhotos = computed(() => {
  return Object.values(props.photoRequirements).every(req => req.has_minimum)
})

const missingPhotoStages = computed(() => {
  return Object.entries(props.photoRequirements)
    .filter(([_, req]) => !req.has_minimum)
    .map(([stage, _]) => stage)
})

// Submit form
const submit = () => {
  form.post(route('completion.store', props.job.id))
}
  ...
  <AppLayout>
    ...
    <div class="max-w-5xl space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-bold tracking-tight">Create Completion Report</h1>
      ...
      >
        ...
        <AlertDescription>
          ...
          You need to upload photos for the following stages before submitting:
          {{ missingPhotoStages.join(', ') }}
          <Link :href="route('jobs.photos.index', job.id)" class="underline ml-2">
            Upload Photos
          </Link>
        </AlertDescription>
      </Alert>
      ...
      <Alert v-else class="border-green-500 bg-green-50">
        ...
        <AlertDescription>
          All required photos have been uploaded. You can proceed with the completion report.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        ...
        <CardContent class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div v-if="job.assigned_user">
            <Label class="text-muted-foreground">Assigned Technician</Label>
      ...
      <form @submit.prevent="submit">
        ...
        <Card class="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Provide comprehensive details about the repair work completed
            </CardDescription>
          </CardHeader>
          <CardContent class="space-y-6">
            ...
            <div class="space-y-2">
              <Label for="time_spent_hours">
                Time Spent (Hours) <span class="text-destructive">*</span>
              </Label>
              <Input
                id="time_spent_hours"
                ...
                min="0.1"
                placeholder="e.g., 8.5"
                :class="{ 'border-destructive': form.errors.time_spent_hours }"
              />
              <p v-if="form.errors.time_spent_hours" class="text-sm text-destructive">
            ...
            <div class="space-y-2">
              <Label for="work_description">
                Work Description <span class="text-destructive">*</span>
              </Label>
              <Textarea
                id="work_description"
                ...
                rows="5"
                :class="{ 'border-destructive': form.errors.work_description }"
              />
              <p v-if="form.errors.work_description" class="text-sm text-destructive">
                {{ form.errors.work_description }}
              </p>
              <p class="text-sm text-muted-foreground">
                Include specific repairs, procedures followed, and any relevant technical details
              </p>
            </div>

            <!-- Issues Encountered -->
            <div class="space-y-2">
              <Label for="issues_encountered">Issues Encountered (Optional)</Label>
              <Textarea
              ...
              />
              <p class="text-sm text-muted-foreground">
                Document any problems that arose during the repair process
              </p>
            </div>
            ...
            <div class="space-y-2">
              <Label for="quality_rating">
                Quality Rating <span class="text-destructive">*</span>
              </Label>
              ...
              <p v-if="form.errors.quality_rating" class="text-sm text-destructive">
                {{ form.errors.quality_rating }}
              </p>
              <p class="text-sm text-muted-foreground">
                Rate the overall quality of the repair work completed
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card class="mb-6">
          <CardHeader>
            <div class="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Track all parts and materials used during the repair
                </CardDescription>
              </div>
              <Button
                type="button"
          ...
          <CardContent>
            <Alert v-if="parts.length === 0" variant="default" class="mb-4">
              <AlertCircle class="h-4 w-4" />
              <AlertDescription>
                At least one part must be added. Click "Add Part" to begin.
              </AlertDescription>
            </Alert>
            ...
            <div v-if="parts.length > 0">
              <Table>
                ...
                <TableBody>
                  <TableRow v-for="(part, index) in parts" :key="index">
                    <TableCell class="font-medium">{{ part.name }}</TableCell>
                    <TableCell class="text-right">{{ part.quantity }}</TableCell>
                    <TableCell class="text-right">{{ formatCurrency(part.cost) }}</TableCell>
                  ...
                  <TableRow>
                    <TableCell colspan="3" class="text-right font-bold">Total Cost:</TableCell>
            ...
            <p v-if="form.errors.parts_used" class="text-sm text-destructive mt-2">
        ...
        <Card class="mb-6">
          ...
          <CardContent>
              ...
              >
                ...
                <div class="flex items-center gap-2">
                  <Badge variant="secondary">
                    {{ req.count }} / {{ req.minimum_required }} photos
                  </Badge>
                  <CheckCircle
            ...
            <Link
              :href="route('jobs.photos.index', job.id)"
        ...
        <div class="flex items-center justify-end gap-4">
          ...
          >
            ...
            {{ form.processing ? 'Saving...' : 'Save Completion Report' }}
          </Button>
        </div>
      </form>
    </div>
    ...
    <Dialog v-model:open="showAddPartDialog">
      <DialogContent>
        ...
        <div class="space-y-4 py-4">
          ...
          <div class="space-y-2">
            <Label for="part_cost">
              Unit Cost (MYR) <span class="text-destructive">*</span>
            </Label>
            <Input
              id="part_cost"
          ...
          <div v-if="partForm.quantity > 0 && partForm.cost > 0" class="pt-2 border-t">
            <div class="flex justify-between items-center">
              <span class="font-medium">Total Cost:</span>
              <span class="text-lg font-bold">
                {{ formatCurrency(partForm.quantity * partForm.cost) }}
              </span>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            @click="addPart"
            :disabled="!partForm.name || partForm.quantity <= 0 || partForm.cost < 0"
          >
            Add Part
          </Button>
        </DialogFooter>
