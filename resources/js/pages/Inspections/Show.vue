...
const editForm = useForm({
  ...
  functional_testing_results: props.inspection.functional_testing_results || '',
  safety_hazards_identified: props.inspection.safety_hazards_identified || '',
  additional_issues_discovered: props.inspection.additional_issues_discovered || '',
  recommended_repairs: props.inspection.recommended_repairs || '',
})

// Submit approval
const submitApproval = () => {
  approveForm.post(route('inspections.approve', props.inspection.id), {
    onSuccess: () => {
...
const submitApprovalWithConditions = () => {
  approveConditionsForm.post(route('inspections.approve-conditions', props.inspection.id), {
    onSuccess: () => {
...
const submitRejection = () => {
  rejectForm.post(route('inspections.reject', props.inspection.id), {
    onSuccess: () => {
...
// Submit edit
const submitEdit = () => {
  editForm.put(route('inspections.update', props.inspection.id), {
    onSuccess: () => {
...
<template>
  <AppLayout>
    <Head :title="`Inspection - ${inspection.workshop_job.job_reference}`" />
    ...
    <div class="max-w-6xl space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold tracking-tight">Inspection Report</h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <Label class="text-muted-foreground">Job Reference</Label>
                  <Link
                    :href="route('jobs.show', inspection.workshop_job.id)"
                    class="font-medium text-primary hover:underline"
                  >
                    {{ inspection.workshop_job.job_reference }}
                  </Link>
                </div>
                <div v-if="inspection.workshop_job.kew_pa_10">
                  <Label class="text-muted-foreground">KEW.PA-10 Number</Label>
                  <p class="font-medium">{{ inspection.workshop_job.kew_pa_10.kew_pa_10_number }}</p>
                </div>
              </div>

              <div v-if="inspection.workshop_job.asset">
                <Label class="text-muted-foreground">Asset</Label>
                <p class="font-medium">{{ inspection.workshop_job.asset.asset_name }}</p>
                <p class="text-sm text-muted-foreground">
                  {{ inspection.workshop_job.asset.asset_tag }} - {{ inspection.workshop_job.asset.asset_type }}
                </p>
              </div>

              <div v-if="inspection.workshop_job.government_department">
                <Label class="text-muted-foreground">Government Department</Label>
                <p class="font-medium">{{ inspection.workshop_job.government_department.name }}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent class="space-y-4">
              <div v-if="inspection.asset_condition_current">
                <Label class="text-muted-foreground">Current Asset Condition</Label>
                <p class="mt-1">{{ inspection.asset_condition_current }}</p>
              </div>

              <div v-if="inspection.visual_damage_assessment">
                <Label class="text-muted-foreground">Visual Damage Assessment</Label>
                <p class="mt-1">{{ inspection.visual_damage_assessment }}</p>
              </div>

              <div v-if="inspection.functional_testing_results">
                <Label class="text-muted-foreground">Functional Testing Results</Label>
                <p class="mt-1">{{ inspection.functional_testing_results }}</p>
              </div>

              <div v-if="inspection.safety_hazards_identified">
                <Label class="text-muted-foreground">Safety Hazards Identified</Label>
                <p class="mt-1">{{ inspection.safety_hazards_identified }}</p>
              </div>

              <div v-if="inspection.additional_issues_discovered">
                <Label class="text-muted-foreground">Additional Issues Discovered</Label>
                <p class="mt-1">{{ inspection.additional_issues_discovered }}</p>
              </div>

              <div v-if="inspection.recommended_repairs">
                <Label class="text-muted-foreground">Recommended Repairs</Label>
                <p class="mt-1">{{ inspection.recommended_repairs }}</p>
              </div>
            </CardContent>
          </Card>

          <!-- Inspection Photos -->
          <Card v-if="inspection.photos && inspection.photos.length > 0">
            <CardHeader>
        ...
        <div class="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <Badge :variant="getStatusBadgeVariant(inspection.approval_status)" class="text-base">
          ...
          <Card>
            ...
            <CardContent class="space-y-2">
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium">{{ inspection.inspector.name }}</span>
              </div>
              <p class="text-sm text-muted-foreground">{{ inspection.inspector.email }}</p>
            </CardContent>
          </Card>
      ...
      <DialogContent>
        ...
        <div class="space-y-4 py-4">
          <Alert>
            ...
            <AlertDescription>
              By approving, you confirm that the inspection findings are accurate and
              the recommended repairs are appropriate.
            </AlertDescription>
          </Alert>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            :disabled="approveForm.processing || !approveForm.digital_signature"
          >
            {{ approveForm.processing ? 'Approving...' : 'Approve Inspection' }}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            :disabled="approveConditionsForm.processing || !approveConditionsForm.conditions || !approveConditionsForm.digital_signature"
          >
            {{ approveConditionsForm.processing ? 'Approving...' : 'Approve with Conditions' }}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
      ...
      <DialogContent>
        ...
        <div class="space-y-4 py-4">
          <Alert variant="destructive">
            <XCircle class="h-4 w-4" />
            <AlertDescription>
              Rejecting will require a new inspection to be performed.
        ...
        <DialogFooter>
          ...
          <Button
            ...
            :disabled="rejectForm.processing || !rejectForm.reason || !rejectForm.digital_signature"
          >
            {{ rejectForm.processing ? 'Rejecting...' : 'Reject Inspection' }}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
